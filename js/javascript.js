      ...
      $.notify(
        {
          message:
            ...
            lang.js_save_file_for_quick_download +
            "</span>",
        },
        {
          type: "danger",
          ...
          animate: { enter: "animated zoomIn", exit: "animated zoomOut" },
        }
      );
    } else {
      $.post("/AI/d", { file_id: $(this).attr("data-id") }).done(function (
        data
      ) {});
    }
  });
...
function AiD(file_id) {
  $.post("/AI/d", { file_id: file_id }).done(function (data) {});
}
function reloadIFrame() {
  document.getElementById("viewerPro").src =
    document.getElementById("viewerPro").src;
}
...
function previewPro() {
  ...
  $(".preview-link, #previewButtonMain").click(function (e) {
    if ($("#viewerPro").attr("src") != $(this).attr("data-preview")) {
      ...
      timerId = setInterval(function () {
        if (iframeLoadingTrials <= 0) {
          $("#viewerPro").attr(
            "src",
            previewUrl + "&trial" + iframeLoadingTrials
          );
          console.log(
            "Load again: " +
              $("#viewerPro").attr("src") +
    ...
    if ($(this).hasClass("in-cloud-2")) {
      ...
      $("#goToFileButtonMemberModal").attr(
        "href",
        $(this).attr("data-download-page")
      );
    }
    ...
    function () {
      ...
      var SetRatingStar = function () {
        return $star_rating.each(function () {
          if (
            parseInt($star_rating.siblings("#input-rate-" + target).val()) >=
    ...
    $(document).on({
      mouseleave: function (e) {
        if (e.clientY < 0 && getCookie("pdfdriveAlerts") == "") {
          $("#pdfdriveAlertsImg").attr(
            "src",
            "/assets/img/pd-trending-mail.jpg"
          );
...
$(document).ready(function () {
  ...
  window.onpopstate = function (event) {
    if ($(".mobile-menu").hasClass("mobile-menu--open")) {
      ...
      if (window.location.hash == "" && just_loaded) window.history.back();
    }
  };
});
function askYourself() {
  $("#ask-yourself")
    .fadeTo(300, 0)
    .load("/home/askYourself", {}, function () {
...
function validate(id) {
  if (validateEmail($("#email-" + id).val())) {
    $.getScript(
      "https://www.google.com/recaptcha/api.js?render=6Lfo3eoUAAAAAG1agFKNm0HMJiuml_Dee8kdr1nz"
    ).done(function (script, textStatus) {
      grecaptcha.ready(function () {
        grecaptcha
          .execute("6Lfo3eoUAAAAAG1agFKNm0HMJiuml_Dee8kdr1nz", {
            action: "emailSignup",
          })
          .then(function (token) {
            $.post("/mail/addMail", {
...
function rd() {
  if (navigator.cookieEnabled) {
    ...
    if (getCookie("redirected") == "") redirected = 0;
    if (!vis() && redirected == 0) {
      try {
      } catch (err) {}
      if (redirected == 0) {
...
function fileInfoForMobile(force) {
  if (window.innerWidth <= 640 || force) {
    $(".file-info").each(function (i) {
      $("#file-info-" + $(this).attr("data-id")).html(
        $(this).children(".pages").html().toLowerCase() +
          " Â· " +
          $(this).children(".year").html() +
          " Â· " +
          $(this).children(".size").html() +
          " Â· " +
          $(this).children(".downloads").html().toLowerCase() +
          " Â· "
      );
    });
  }
...
$(document).ready(function () {
  $(".img-zoom").hoverIntent({ over: makeLarge, out: makeSmall });
  getFileNumber();
});
var orjwidth = "116px";
var orjheight = "160px";
...
function animateValue(id, start, end, duration) {
  if (Math.abs(end - start) > 200) return;
  ...
  function run() {
    ...
    if (!$("#q").is(":focus"))
      $("#q").attr(
        "placeholder",
        sprintf(lang.js_search_for_books, numberWithCommas(value))
      );
    if (value == end) {
      cend = end;
      clearInterval(timer);
...
function getFileNumber() {
  $.ajax({ url: "/ebook/counter", data: { time: Math.random() } }).done(
    function (data) {
      if (cend == 0) {
        cend = data;
